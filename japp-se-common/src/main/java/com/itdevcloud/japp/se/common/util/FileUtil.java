...
public class FileUtil {
	...
	public static void copy(String source, String destination, FileFilter filter) throws IOException {
		if (source == null || source.trim().equals("") || destination == null || destination.trim().equals("")) {
			return;
		}
		...
		if (inputFile.isFile()) {
			...
			if (outputFile.isFile()) {
				throw new IOException("Scource file < " + source + " > is a directory, so " + "destination < "
	...
	private static void copyFile(String source, String destination) throws IOException {
		...
		try {
			...
			if (!inputFile.isFile() || outputFile.isDirectory()) {
				throw new IOException("Scource file < " + source + " > should be a file and " + "destination < "
						+ destination + " > should not be a dictionary. ");
			}
			...
			throw new IOException("Exception occurs during file copy process! " + "scource file < " + source
					+ ", destination > " + destination + " >. " + "Detail information: \n" + e);
		} finally {
			if (in != null) {
				try {
					in.close();
	...
	private static void copyDir(String source, String destination, FileFilter filter) throws IOException {
		if (source == null || source.trim().equals("") || destination == null || destination.trim().equals("")) {
			return;
		}
		...
		if (!inputFile.isDirectory() || outputFile.isFile()) {
			throw new IOException("Scource file < " + source + " > should be a directory and " + "destination < "
	 ...
	 *
	 */
	public static boolean rename(String oldFileOrDir, String newFileOrDir) throws IOException {
		if (oldFileOrDir == null || oldFileOrDir.trim().equals("") || newFileOrDir == null
				|| newFileOrDir.trim().equals("")) {
			return true;
		}
	...
	public static boolean move(String oldFileOrDir, String destDir) throws IOException {
		if (oldFileOrDir == null || oldFileOrDir.trim().equals("") || destDir == null || destDir.trim().equals("")) {
			return true;
		}
	...
	public static Map<String, String> getFileListingInClassPath(String path, boolean includeSubPackage,
			FileFilter fileFilter) {
		return getFileListingInClassPath(null, path, includeSubPackage, fileFilter);
	}

	public static Map<String, String> getFileListingInClassPath(Class clazz, String path, boolean includeSubPackage,
			FileFilter fileFilter) {
		...
		try {
			if (dirURL != null && dirURL.getProtocol().equals("file")) {
				...
				/* A JAR path */
				String zipPath = dirURL.getPath().substring(0, dirURL.getPath().indexOf("!"));
				...
				logger.warning("FileUtil.getFileListingInClassPath()... does NOT support this Protocol - "
						+ dirURL.getProtocol());
			} // end if-else
		} catch (Throwable t) {
			t.printStackTrace();
	...
	public static List<Class<?>> getClassesInPackage(String pkgName) {
		...
		if (directory != null && directory.exists()) {
			...
			try {
				...
				while (entries.hasMoreElements()) {
					...
					if (entryName.startsWith(resourcePath)
							&& entryName.length() > (resourcePath.length() + "/".length())
			...
			throw new RuntimeException(
					"getClassesInPackage() - only support local directory or jar files. Code need to be enhance to deal with this kind of path: "
							+ fullPath);
		}
