...
public class PkiService implements AppFactoryComponentI {
	...
	private Key piscesjappPrivateKey;
	private PublicKey piscesjappPublicKey;
	private Certificate piscesjappCertificate;
	private boolean comeFromKeyVault = false;

	//key name in Azure Key Vault
	@Value("${PiscesJappKeys:}")
	...
	public void retrievePiscesJappKeyPair() {
		...
		try {
			...
			if (ConfigFactory.appConfigService.getPropertyAsBoolean(AppConfigKeys.AZURE_KEYVAULT_ENABLED)) {
				logger.info(
						"retrievePiscesJappKeyPair() try to load PISCESJAPP private and public key from Azure Key Vault............");
				comeFromKeyVault = true;
//				String encodedPkcs12Str = piscesjappConfigService.getPropertyAsString(PiscesJappConfigKeys.AZURE_KEYVAULT_JAPPCORE_PKCS12_KEY);
				String encodedPkcs12Str = getAzureKeyVaultKeys();
				byte[] decodedPkcs12Bytes = Base64.getDecoder().decode(encodedPkcs12Str);
				//log.info("retrievePiscesJappKeyPair() .....Pkcs12Str...." + encodedPkcs12Str);
				...
				if (aliasList.isEmpty()) {
					piscesjappPrivateKey = null;
					piscesjappPublicKey = null;
					piscesjappCertificate = null;
				...
				if (StringUtils.isEmpty(keyAlias)) {
					if (aliasList.size() == 1) {
						...
						logger.warn("retrievePiscesJappKeyPair() - There is no keyAlias defined in property file, use only alias defined in pkcs12 stream, the alias name = " + alias);
						piscesjappPrivateKey = keystore.getKey(alias, kepPassword.toCharArray());
						piscesjappCertificate = keystore.getCertificate(alias);
						piscesjappPublicKey = piscesjappCertificate.getPublicKey();
					...
					for (String alias : aliasList) {
						if (keyAlias.equalsIgnoreCase(alias)) {
							logger.info("retrievePiscesJappKeyPair() .....alias name = " + alias);
							piscesjappPrivateKey = keystore.getKey(alias, kepPassword.toCharArray());
							piscesjappCertificate = keystore.getCertificate(alias);
							piscesjappPublicKey = piscesjappCertificate.getPublicKey();
							foundAlias = true;
				...
				//log.info("retrievePiscesJappKeyPair()...Azure Key Vault...piscesjappPublicKey=" + piscesjappPublicKey);
			} else {
				String privateKeyStore = getPrivateKeyStore();
				logger.info("retrievePiscesJappKeyPair() try to load PISCESJAPP private and public key in " + privateKeyStore
						+ " from classpath.....");
				...
				if (resource == null) {
					...
					logger.info("retrievePiscesJappKeyPair().......privateKeyStore=" + springPrivateKS);
				}
				if (resource == null) {
					String err ="retrievePiscesJappKeyPair() can not get keystore resource, check code! privateKeyStore=" + privateKeyStore;
					logger.error(err);
					throw new RuntimeException(err);
				}
				...
				//logger.info("retrievePiscesJappKeyPair().....FileInputStream =" + is);
				...
				piscesjappPrivateKey = keystore.getKey(ConfigFactory.appConfigService.getPropertyAsString(AppConfigKeys.SERVER_SSL_KEY_ALIAS),
						ConfigFactory.appConfigService.getPropertyAsString(AppConfigKeys.SERVER_SSL_KEY_PASSWORD).toCharArray());
				// log.info("retrievePiscesJappKeyPair().....piscesjappPrivateKey==" + piscesjappPrivateKey);
				piscesjappCertificate = keystore.getCertificate(ConfigFactory.appConfigService.getPropertyAsString(AppConfigKeys.SERVER_SSL_KEY_ALIAS));
//				logger.info("PiscesJappKeyService.retrievePiscesJappKeyPair().....piscesjappCertificate =" + piscesjappCertificate);
				piscesjappPublicKey = piscesjappCertificate.getPublicKey();
//				logger.info("PiscesJappKeyService.retrievePiscesJappKeyPair().....piscesjappPublicKey =" + piscesjappPublicKey.getClass());
			}
		} catch (Exception e) {
			piscesjappPrivateKey = null;
			piscesjappPublicKey = null;
	...
	public void setPiscesJappCertificate(Certificate piscesjappCertificate) {
		this.piscesjappCertificate = piscesjappCertificate;
	}
